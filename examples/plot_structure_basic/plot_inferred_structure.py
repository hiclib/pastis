...
##############################################################################
# Get a dataframe for the inferred structure's coordinates.
df = pd.read_csv('data/struct_inferred.000.coords', delimiter=" ", header=None)
...
def scatterBeads(x, y, z, bead_size, name, curr_cmap, indices, ax):
    last_idx = len(x) - 1
...
##############################################################################
# Turn the dataframe into an array of coordinates for the two chromosomes,
# both of which have length 60 (the array is length 120). Then, get the
# coordinates for each chromosome.
coordArr = np.array(df)
theLengths = np.array([60, 60])

# Get the first chromosome's coordinates
start, end = 0, theLengths[0]
x1, y1, z1 = coordArr[:,0][start:end], coordArr[:,1][start:end], coordArr[:,2][start:end]

# Get the second chromosome's coordinates
start, end = end, end + theLengths[1]
x2, y2, z2 = coordArr[:,0][start:end], coordArr[:,1][start:end], coordArr[:,2][start:end]



##############################################################################
# Interpolate the coordinates of the chromosomes.
chrom1, chrom2 = interpolate_chromosomes(coordArr, theLengths)
...
# Get our colors set up. color_indices determines which color in our color map
# each bead will have.
...
scatterBeads(x1, y1, z1, 50, 'chromosome 1', cmap1, color_beads, ax)
scatterBeads(x2, y2, z2, 50, 'chromosome 2', cmap2, color_beads, ax)
